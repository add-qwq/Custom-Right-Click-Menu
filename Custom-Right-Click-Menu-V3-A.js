...
// 此文件为CRCM.V3-A.js的格式化版本，若在意加载速度，则不建议在生产环境中使用
...
class CustomRightClickMenu extends HTMLElement {
    ...
    _renderMenuLayer(items, parentContainer, ctx) {
        items.forEach((item) => {
            ...
            menuItem.addEventListener('mouseenter', () => {
                const siblings = parentContainer.querySelectorAll('.menu-item');
                    ...
                    menuItem.addEventListener('mouseenter', () => {
                        ...
                        if (canFitRight) {
                            ...
                            if (canFitDown) {
                                ...
                                subMenu.style.top = `-${subHeight}px`;
                            }
                            return;
                        }

                        let currentTop = -5;
                        let absoluteBottom = parentRect.top + currentTop + subHeight;

                        if (absoluteBottom + safeMargin > viewportHeight) {
                            const overflow = (absoluteBottom + safeMargin) - viewportHeight;
                            currentTop -= overflow;
                        }

                        if (parentRect.top + currentTop < safeMargin) {
                            currentTop = safeMargin - parentRect.top;
            ...
            menuItem.addEventListener('mouseleave', (e) => {
                ...
                if (!related || !this.shadowRoot.contains(related)) {
                    const allSubs = this.shadowRoot.querySelectorAll('.sub-menu');
                    allSubs.forEach(s => s.style.visibility = '');
                }
            });
    ...
    handleClickOutside(e) {
        ...
        if (this.isMenuVisible && !path.includes(this.customMenu)) {
            this.hideMenu();
        }
    }
    handleScroll() {
